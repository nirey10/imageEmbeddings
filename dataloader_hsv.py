...
def download_and_unzip(URLL, root_dir):
  error_message = "Download is not yet implemented. Please, go to {URL} urself."
  raise NotImplementedError(error_message.format(URLL))
...
class TinyImageNetDataset(Dataset):
  ...
  def __getitem__(self, idx):
    ...
    img[rand_x: rand_x + patch_size, rand_y: rand_y + patch_size, :] = self.increase_brightness(img[rand_x: rand_x + patch_size, rand_y: rand_y + patch_size, :], intensity_addition)

    hsv_img = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
    hsv_orig_img = cv2.cvtColor(orig_img, cv2.COLOR_BGR2HSV)

    if self.transform:
      hsv_img = self.transform(hsv_img)
      hsv_orig_img = self.transform(hsv_orig_img)
    return (hsv_orig_img, hsv_img, lbl)